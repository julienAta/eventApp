...
describe("Expense Tests", () => {
  // Unit tests
  describe("Unit Tests", () => {
    test("getExpenses returns all expenses", async () => {
      const mockExpenses = [
        {
          id: 1,
          event_id: 1,
      ...
      const result = await expenseModel.getExpenses();
      expect(result).toEqual(mockExpenses);
    });

    test("addExpense creates a new expense", async () => {
      const newExpense = {
        event_id: 1,
      ...
      const result = await expenseModel.addExpense(newExpense);
